...
function ScheduleCard({ candidate, job, onSchedule, scheduled }) {
    ...
    <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-4">
      ...
      <AnimatePresence>
        {open && !scheduled && (
          ...
          >
            ...
            {/* Confirm */}
            <button
              onClick={() => onSchedule(candidate.candidate_id, { type, interviewerObj, slot, interviewType })}
              className="w-full py-2 rounded-lg bg-green-600 hover:bg-green-500 text-white text-xs font-medium transition-colors"
            >
              📅 Send Calendar Invite to {candidate.name}
            </button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {scheduled && (
        <div className="mt-2 text-slate-500 text-xs">
          {scheduled.interviewType.label} with {scheduled.interviewerObj.name} on{' '}
...
export default function SchedulePanel({ candidates, jobs }) {
      ...
      <div className="bg-indigo-900/20 border border-indigo-800/40 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-indigo-200/70 text-xs mt-0.5 leading-5">
            ...
            and interviewer automatically. This demo simulates that UX.
          </p>
        </div>
      </div>
